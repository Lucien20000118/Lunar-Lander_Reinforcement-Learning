...
class ContactDetector(contactListener):
    ...
    def BeginContact(self, contact):
        ...
        categoryA = contact.fixtureA.filterData.categoryBits
        categoryB = contact.fixtureB.filterData.categoryBits


        if bodyA == self.env.lander or bodyB == self.env.lander:
            self.env.game_over = True
            # print(f"Lander collided with an object of {categoryA},{categoryB}")

        for i in range(2):
            if self.env.legs[i] in [bodyA, bodyB]:
                # print(f"Lander legs contact with an object of {categoryA},{categoryB}")
...
class LunarLander(gym.Env, EzPickle):
    ...
    * The angular velocity is in units of 0.4 radians per second. In order to convert to radians per second, the value needs to be multiplied by a factor of 2.5.
    ...
    After the correction has been made, the units of the state are as folCreatefllows:
    'x': (units)
    'y': (units)
    'vx': (units/second)
    'vy': (units/second)
    ...
    def _destroy(self):
        ...
        self.lander = None
        for _ in self.obstacle:
            self.world.DestroyBody(_)
        self.obstacle = None
    ...
    def reset(
        ...
        *,
        ...
        # height[CHUNKS // 2 + 2] = self.helipad_y
        w=8
        self.helipad_x1 = chunk_x[w-1] 
        self.helipad_x2 = chunk_x[w+1]
        self.helipad_y = H / 4
        height[w - 2] = self.helipad_y
        height[w - 1] = self.helipad_y+0.5
        height[w + 0] = self.helipad_y+1
        height[w + 1] = self.helipad_y+1.5
        height[w + 2] = self.helipad_y+2
        smooth_y = [
        ...
        for i in range(CHUNKS - 1):
            ...
            self.moon.CreateEdgeFixture(vertices=[p1, p2], density=0, friction=10)
        ...
        CAREGORY_OBSTACLE = 5
        self.obstacle = []
        obstacle_xposition = [0 , 0.5]
        obstacle_yposition = [0 , 0.2]
        for _ in range(2):
            obstacle = self.world.CreateStaticBody(
                position=(VIEWPORT_W / SCALE / 2 + obstacle_xposition[_] * (VIEWPORT_W / SCALE / 2), VIEWPORT_H / SCALE / 1.5 + obstacle_yposition[_] * (VIEWPORT_H / SCALE / 1.5)),
                fixtures=fixtureDef(
                    shape=polygonShape(box=(0.5, 0.1)),
                    categoryBits=CAREGORY_OBSTACLE,  
                    maskBits = CAREGORY_BODY | CAREGORY_LEGS,
                ),
                userData={'color': 'obstacle'}
            )
    ...
    def step(self, action):
        ...
        ## heading-engine    
        m_power_reverse = 0.0  # 初始化反向引擎的功率
        
        if (self.continuous and action[0] > 0.0) or (
            not self.continuous and action == 4
        ):
            m_power_reverse = 1.0
            ...
            impulse_pos_reverse = (self.lander.position[0] + ox_reverse, self.lander.position[1] + oy_reverse)
            
            
            self.lander.ApplyLinearImpulse(
                (-ox_reverse * MAIN_ENGINE_POWER * m_power_reverse, -oy_reverse * MAIN_ENGINE_POWER * m_power_reverse),
                impulse_pos_reverse,
                True,
            )
            ...
            if self.render_mode is not None:
                p_reverse = self._create_particle(
                    3.5, 
                    impulse_pos_reverse[0],
                    impulse_pos_reverse[1],
                    m_power_reverse,
                )
                p_reverse.ApplyLinearImpulse(
                    (
                        ox_reverse * MAIN_ENGINE_POWER * m_power_reverse,
                        oy_reverse * MAIN_ENGINE_POWER * m_power_reverse,
                    ),
                    impulse_pos_reverse,
                    True,
                )   
        ...
        ##### modify x_axis reward function
        state = [
            (pos.x - (self.helipad_x1+self.helipad_x2)/2) / (VIEWPORT_W / SCALE / 2),
            (pos.y - (self.helipad_y + 1 + LEG_DOWN / SCALE)) / (VIEWPORT_H / SCALE / 2),
        ...
        if self.legs[0].ground_contact :
            if self.legs[1].ground_contact:
                if pos.x>self.helipad_x1 and pos.x<self.helipad_x2:
                    if self.pre_pos_x is not None and \
                        self.pre_pos_x > self.helipad_x1 and \
                        self.pre_pos_x < self.helipad_x2:
                        terminated = True 
                        reward = reward + 100
                        reach_goal = True
                
                
        self.pre_pos_x  = pos.x
        self.pre_pos_y  = pos.y   
        ...
        return np.array(state, dtype=np.float32), reward, terminated, reach_goal, {}
    ...
    def render(self):
        ...
        for obj in self.particles + self.drawlist:
            for f in obj.fixtures:
                ...
                for idx,(x,y) in enumerate([(self.helipad_x1, self.helipad_y), (self.helipad_x2, self.helipad_y)]):
                    x = x * SCALE
                    flagy1 = y * SCALE +10
                    flagy2 = flagy1 + 50
                    if idx ==1:
        ...
        if self.obstacle:
            for obstacle in self.obstacle:
                for fixture in obstacle:
                    shape = fixture.shape
                    vertices = [(obstacle.transform * v) * SCALE for v in shape.vertices]
                    vertices = [(v[0], VIEWPORT_H - v[1]) for v in vertices]  # converting to pygame coordinates
                    pygame.draw.polygon(self.surf, (255, 0, 0), vertices)  # drawing with red color
